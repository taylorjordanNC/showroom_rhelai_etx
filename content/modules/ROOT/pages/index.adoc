...
Now that we done with our document conversion, we can exit out of the shell by simply typing the exit command:
...
However, we want you to understand, at a high level, what a “full training process” is composed of. 

There are two primary phases:

* **Phase 1**: We take the starter model (granite-7b-starter), and fine-tune it with the synthetic knowledge dataset produced from SDG. This training process outputs 4 model ‘checkpoints’. The checkpoints are evaluated. The best model checkpoint goes to Phase 2.

* **Phase 2**: Our model output from phase 1 is now trained on skills data. This skills data is based on data pre-loaded into the RHEL AI product and is focused on ensuring our final model can process, understand, and answer in the most accurate way from the given context. This process outputs 4 new checkpoints. The checkpoints are again evaluated and the ‘best’ one becomes our final model.
